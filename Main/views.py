...
def index(request):
    return render(request, 'index.html',{'sort_list': [],'meta_list' : meta_data_mgr.metalist})
...
def sort_bubble(request):
    ...
    for i_item in sortList[:-1]:	    #遍历0至N-1的列表切片；list长度为N，因为从0开始，所以需要减1，实际进行了N-1趟循环（0~N-2）
        ...
        i+=1
    utils.PrintList(sortList)
    return render(request, 'sort_bubble.html',{'sort_list': sortList,'meta_list' : meta_data_mgr.metalist})
...
def ajax_tool(request):
    ...
    return render(request, 'data.html',{'sort_list': [],'meta_list' : meta_data_mgr.metalist})